...
struct ResultsTableView: View {
                ...
                LazyVStack(spacing: 0, pinnedViews: [.sectionHeaders]) {
                    Section {
                        if rows.isEmpty {
                            ...
                            ForEach(rows) { row in
                                dataRow(row, widths: widths, totalWidth: contentW)
                            }
                            let used = CGFloat(rows.count) * rowHeight
                            let fill = max(0, geo.size.height - headerHeight - used)
                            if fill > 0 {
    ...
    private func dataRow(_ row: ResultTableRow, widths: [CGFloat], totalWidth: CGFloat) -> some View {
        HStack(spacing: 0) {
            Text("\(row.number)")
            ...
            ForEach(Array(model.columns.enumerated()), id: \.element.id) { index, column in
                ...
                Group {
                    if isEditing {
                        ...
                        cell(row.value(at: column.ordinal), selectable: !isEditable)
                    }
                }
    ...
    @ViewBuilder
    private func cell(_ value: SQLValue, selectable: Bool) -> some View {
        if value == .null {
            Text("NULL")
                .font(.system(size: 12.5, design: .monospaced))
                .italic()
            ...
            let text = Text(value.displayString)
                .font(.system(size: 12.5, design: .monospaced))
                .foregroundStyle(.primary)
                .lineLimit(1)
                .truncationMode(.middle)
                .frame(maxWidth: .infinity, alignment: .leading)
            // textSelection steals mouse events and breaks click-to-edit.
            if selectable {
